...
def bw_config(bw):
    if bw == "EXT BW":
        app.ev12aq600_ext_bw_disable(0)
    else:
        app.ev12aq600_ext_bw_disable(1)
"""  
#################################################################################
## PLOT FUNCTIONS 
#################################################################################
"""
def core_fft(core_list, f_s, adc_resolution, window_enable, window_select):
    ...
    if window_enable:
        if window_select == "blackman":
            w = blackman(N)
        elif window_select == "hanning":
            w = hanning(N)
...
def plot_iladata():
    ...
    if ch_mode == 1 and clk_mode != 3:
        ...
        print (">> Create fft")
        #########################################
        xfa, yfa = core_fft(core_data[0], f_s, adc_resolution, window_enable, window_select) 
        xfb, yfb = core_fft(core_data[1], f_s, adc_resolution, window_enable, window_select) 
        xfc, yfc = core_fft(core_data[2], f_s, adc_resolution, window_enable, window_select) 
        xfd, yfd = core_fft(core_data[3], f_s, adc_resolution, window_enable, window_select) 
        if clk_mode == 3:
            xf, yf = core_fft(data_averaging, f_s, adc_resolution, window_enable, window_select)
        #########################################
        ## plot fft
        #########################################
...
##-------------------------------------------------------------------------------
e32_options_list = ["NOM BW", "EXT BW"]
e32v = StringVar(root)
e32v.set("EXT BW")
e32 = tkinter.OptionMenu(root, e32v, *e32_options_list, command=bw_config)
e32.config(width=menu_width)
e32.grid(row = r, column = 2)
##-------------------------------------------------------------------------------
## New row
##-------------------------------------------------------------------------------
r = r + 1
